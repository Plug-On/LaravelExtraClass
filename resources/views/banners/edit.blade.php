...
<form action="{{route('banners.update',$banner->id)}}" method="POST" class="mt-5" enctype="multipart/form-data" >
    ...
    <input type="text" class="block w-full my-5 rounded-lg border-gray-300" placeholder="Enter Banner Title" name="title" value="{{$banner->title}}">
    @error('title')
    <span class="text-red-600 text-sm block -mt-5">{{($message)}}</span>
    @enderror
    <p>Current Picture</p>
    <img src="{{asset('images/banners/'.$banner->photopath)}}" alt="Bannner Image" class="w-52">
    ...
    <div class="flex justify-center gap-3">
        ...
        <a href="{{route('banners.index')}}" class="bg-red-600 text-white px-3 py-2 rounded-lg">Exit</a>
